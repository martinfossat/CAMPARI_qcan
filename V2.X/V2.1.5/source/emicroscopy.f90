...
!
  do xj=1,emcoarsegrid%dim(1)
    if (doblocks.EQV..true.) xk = ceiling(xsc*(xj-0.5))
    do yj=1,emcoarsegrid%dim(2)
      if (doblocks.EQV..true.) yk = ceiling(ysc*(yj-0.5))
      do zj=1,emcoarsegrid%dim(3)
        if (doblocks.EQV..true.) zk = ceiling(zsc*(zj-0.5))
        cubesum2 = (embgdensity - emcoarsegrid%dens(xj,yj,zj))*(embgdensity - emcoarsegrid%dens(xj,yj,zj))
        if (doblocks.EQV..true.) emcoarsegrid%rblk(xk,yk,zk) = emcoarsegrid%rblk(xk,yk,zk) + cubesum2
        if (doslice.EQV..true.) emcoarsegrid%rslc(yj) = emcoarsegrid%rslc(yj) + cubesum2
        if (doline.EQV..true.) emcoarsegrid%rlin(yj,zj) = emcoarsegrid%rlin(yj,zj) + cubesum2
      end do
    end do
  end do
!
...
!
  type(t_emgrid) it
  integer xi,yi,zi,xj,yj,zj,imol,ii,pv(3),j,k,l,sh,zk,yk,idx,ints(3)
...
!     so only diagonal terms contribute, i.e., d/dyi (Mn(xi)) is obviously zero
      ...
      emgfls(ii,:) = pv(:)
      if (emgfls(ii,1).le.0) emgfls(ii,1) = emgfls(ii,1) + ceiling(-1.0*(emgfls(ii,1)-1)/it%dim(1))*it%dim(1)
      if (emgfls(ii,2).le.0) emgfls(ii,2) = emgfls(ii,2) + ceiling(-1.0*(emgfls(ii,2)-1)/it%dim(2))*it%dim(2)
      if (emgfls(ii,3).le.0) emgfls(ii,3) = emgfls(ii,3) + ceiling(-1.0*(emgfls(ii,3)-1)/it%dim(3))*it%dim(3)
      if (emgfls(ii,1).gt.it%dim(1)) emgfls(ii,1) = emgfls(ii,1) - floor(1.0*emgfls(ii,1)/it%dim(1))*it%dim(1)
      if (emgfls(ii,2).gt.it%dim(2)) emgfls(ii,2) = emgfls(ii,2) - floor(1.0*emgfls(ii,2)/it%dim(2))*it%dim(2)
      if (emgfls(ii,3).gt.it%dim(3)) emgfls(ii,3) = emgfls(ii,3) - floor(1.0*emgfls(ii,3)/it%dim(3))*it%dim(3)
      do k=1,emsplor
        yi = emgfls(ii,2) + k - sh
        if (yi.gt.it%dim(2)) yi = yi - it%dim(2)
        if (yi.lt.1) yi = yi + it%dim(2)
        yj = ceiling((1.0*yi-0.5)/(1.0*ints(2)))
        temk = embspl(k,2,ii)
        if (doslice.EQV..true.) emcoarsegrid%lslc(yj,idx) = .true.
        if (doblocks.EQV..true.) yk = ceiling(ysc*(yj+shf))
        do l=1,emsplor
          zi = emgfls(ii,3) + l - sh
          if (zi.gt.it%dim(3)) zi = zi - it%dim(3)
          if (zi.lt.1) zi = zi + it%dim(3)
          zj = ceiling((1.0*zi-0.5)/(1.0*ints(3)))
          teml = temk*embspl(l,3,ii)
          if (doline.EQV..true.) emcoarsegrid%llin(yj,zj,idx) = .true.
          if (doblocks.EQV..true.) zk = ceiling(zsc*(zj+shf))
          do j=1,emsplor
            temj = teml*embspl(j,1,ii)
            xi = emgfls(ii,1) + j - sh
            if (xi.gt.it%dim(1)) xi = xi - it%dim(1)
            if (xi.lt.1) xi = xi + it%dim(1)
            xj = ceiling((1.0*xi-0.5)/(1.0*ints(1)))
            if (usemass2.EQV..true.) then
              it%mass2(xi,yi,zi) = it%mass2(xi,yi,zi) + temj*tem0
            else
              it%mass(xi,yi,zi) = it%mass(xi,yi,zi) + temj*tem0
            end if
            it%vol(xi,yi,zi) = it%vol(xi,yi,zi) + temj*term0
            if (doblocks.EQV..true.) emcoarsegrid%lblk(ceiling(xsc*(xj+shf)),yk,zk,idx) = .true.
          end do
        end do
      end do
    end do
...
!
  if (soldens.ne.0.0) then
    if (usemass2.EQV..true.) then
      do yi=1,it%dim(2)
        do zi=1,it%dim(3)
          it%mass2(:,yi,zi) = it%mass2(:,yi,zi) - soldens*it%vol(:,yi,zi)
        end do
      end do
    else
      do yi=1,it%dim(2)
        do zi=1,it%dim(3)
          it%mass(:,yi,zi) = it%mass(:,yi,zi) - soldens*it%vol(:,yi,zi)
        end do
      end do
    end if
  end if
...
!
    ...
    if (use_MPIAVG.EQV..true.) then
      ...
      dumpfile = 'N_'//nod(1:tl)//'_DENSITY.nc'
    end if
#else
    dumpfile = 'DENSITY.nc'
#endif
