...
! guess_bonded : whether or not to fudge up length and angle parameters from Engh-Huber
...
!
  integer epsrule,sigrule
  logical vdW_report,elec_report,dip_report,bonded_report,guess_bonded,be_unsafe,charge_patched,bonded_patched
  ...
  RTYPE reduce
!#tyler pka
  RTYPE, ALLOCATABLE:: lj_eps_limits(:,:,:),lj_sig_limits(:,:,:),lj_eps_14_limits(:,:,:),lj_sig_14_limits(:,:,:)
  RTYPE, ALLOCATABLE:: lj_rad_limits(:,:),atsavmaxfr_limits(:,:),atbvol_limits(:,:)
  !integer lj_transform_table(37)! Martin : I kind of want to use this insteadd beacuse I do not like the LJ_limits table the way they are setup
  integer, ALLOCATABLE::  transform_table(:),lj_transform_table(:)! Martin : Added that in the hope to make a one table for all limits
  logical printed_limits
  ! Has to be the size of the number of biotypes
  logical do_pka_2
  logical use_softcore
