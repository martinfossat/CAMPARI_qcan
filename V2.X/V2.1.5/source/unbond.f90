...
!
  implicit none
!
  integer i,j,k,rs,ii,kk,iii,jjj,jj,q,qq
  ...
  RTYPE getblen,getbang,getblen_ref,getbang_ref
!
  delay_report = .false.
  do i=1,n
...
!
      nrsintra(rs) = 0
      do i=1,at(rs)%nbb+at(rs)%nsc ! Martin : goes through all the atoms 
...
!       skip mercilessly
        if ((attyp(ii) .le. 0).or.(attyp(ii).eq.31))  cycle
        do k=i+1,at(rs)%nbb+at(rs)%nsc ! Martin Again goes through all the residues 
          if (k.le.at(rs)%nbb) then
            kk = at(rs)%bb(k)
          else
            kk = at(rs)%sc(k-at(rs)%nbb)
          end if
          if ((attyp(kk) .le. 0).or.(attyp(kk).eq.31)) cycle ! Martin : Important modification for off hydrogens : changed back 
          proceed = .true.
          its14 = .false.
!
...
!       skip mercilessly
        ...
        do k=1,at(rs+1)%nbb+at(rs+1)%nsc
          ...
          if  ((attyp(kk) .le. 0).or.(attyp(kk).eq.31))  cycle
          proceed = .true.
          its14 = .false.
!
...
! Martin : I should focus on that part 
  do rs=1,nseq
!
    nrsbl(rs) = 0
    nrsbleff(rs) = 0
...
!     we have to skip those bonded terms coming form crosslinks
      proceed = .true.
      do k=1,n_crosslinks
        if (rs.eq.crosslink(k)%rsnrs(1)) then
          if ((atmres(tempa(i,1)).eq.crosslink(k)%rsnrs(2)).OR.(atmres(tempa(i,2)).eq.crosslink(k)%rsnrs(2))) then
            proceed = .false.
            exit
          end if
        else if (rs.eq.crosslink(k)%rsnrs(2)) then
          if ((atmres(tempa(i,1)).eq.crosslink(k)%rsnrs(1)).OR.(atmres(tempa(i,2)).eq.crosslink(k)%rsnrs(1))) then
            proceed = .false.
            exit
          end if
        end if
...
!     we have to skip those bonded terms coming form crosslinks
      ...
      do k=1,n_crosslinks
        if (rs.eq.crosslink(k)%rsnrs(1)) then
          if ((atmres(tempb(i,1)).eq.crosslink(k)%rsnrs(2)).OR.(atmres(tempb(i,2)).eq.crosslink(k)%rsnrs(2)).OR.&
 &            (atmres(tempb(i,3)).eq.crosslink(k)%rsnrs(2))) then
            proceed = .false.
            exit
          end if
        else if (rs.eq.crosslink(k)%rsnrs(2)) then
          if ((atmres(tempb(i,1)).eq.crosslink(k)%rsnrs(1)).OR.(atmres(tempb(i,2)).eq.crosslink(k)%rsnrs(1)).OR.&
 &            (atmres(tempb(i,3)).eq.crosslink(k)%rsnrs(1))) then
            proceed = .false.
            exit
          end if
        end if
...
! Martin: I think here is the trick : weird way of doing it 
    do i=1,nrsimpt(rs)
...
!           both are sidechain atoms
          ...
          write(ilog,*) 'Fatal. GROMOS exclusion rules are currently not supported for residue type Y1P (ex PTR). Please &
 &check back later.'
          call fexit()
