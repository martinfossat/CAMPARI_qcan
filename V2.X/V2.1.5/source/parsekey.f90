...
!Martin 
  ...
  else if  (keyword(1:20) .eq. 'FMCSC_DO_SPHERE_BIAS ') then
      read (wo1(wt11:wt12),*) dum
      call dum2int(dum,choice)
      ...
      if (choice.eq.1) then
        ...
        do_sphere_bias=.false.
      end if
   

  else if  (keyword(1:25) .eq. 'FMCSC_SPHERE_BIAS_RADIUS ') then
        read (wo1(wt11:wt12),*) dum
        if (dum.gt.0.) then 
            sphere_bias_radius=dum
        else 
            sphere_bias_radius=0.
        end if 
  
    else if (keyword(1:23) .eq. 'FMCSC_SPHERE_BIAS_FILE ') then !pointer to the file listing the residue that the alternate
      inquire(file=wo1(wt11:wt12),exist=exists)
      if (exists.EQV..true.) then
            sphere_bias_file =  wo1(wt11:wt12)
       end if
