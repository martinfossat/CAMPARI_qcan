...
!
  if (crosslink(i)%itstype.eq.1) then
    if ((rs2.lt.rsmol(molofrs(rs2),1)+2).OR.(rs2.lt.(rs1+2))) then
      write(ilog,*) 'Fatal. Crosslinks between neighboring residues are not supported.'
      call fexit()
    end if
...
! find nine consecutive atoms defining 6 torsional degrees of freedom which we can use to close the chain
!
subroutine crosslink_picksolu(i,i1,i2,i3,i4,i5,i6,i7,i8,i9,solucount,solmat,ccmat,oldvals)
!
  use iounit
  use sequen
  use atoms
...
!
  implicit none
!
  integer i,solucount,i1,i2,i3,i4,i5,i6,i7,i8,i9,rs1,rs2,azero,picked,ptlst(5)
  RTYPE solmat(MAXSOLU*4,6),fc,yc,cc(MAXCHI),ccmat(6),oldvals(12),ba,zt,getztor
!
  azero = 0
  rs1 = crosslink(i)%rsnrs(1)
  rs2 = crosslink(i)%rsnrs(2)
