...
!
 ...
 &         (letter.eq.';').OR.(letter.eq.'_')) then
    ...
    else if ((letter.ne.' ').OR.(isminus.EQV..true.)) then
      exit
    end if
  end do
!
...
!
  if (s2.lt.s1) then
    if ((t2.gt.t1).OR.(string(t1:t1).ne.' ')) then
...
!
subroutine get_ints_from_str(string,nvals,valv,cont,nvalsread)
!
  implicit none
!
  character(*) string
...
!---------------------------------------------------------------------
!
subroutine get_reals_from_str(string,nvals,valv,cont,nvalsread)
!
  implicit none
!
  character(*) string
...
!---------------------------------------------------------------------
!
subroutine get_floats_from_str(string,nvals,valv,cont,nvalsread)
  implicit none
!
  character(*) string
  character(len(string)) string2
...
!  Martin : Took this out because it causes bug and I do not see any use for it
 strsz = max(strsz,minsz)
! althpough it does make the naming convention slghtly differen for the files
