...
! counter structure
  type t_mvcnt
    ...
    integer ndynseg,nmcseg,nhsq,nhs
    RTYPE avgdynseglen,avgmcseglen
  end type t_mvcnt
  type(t_mvcnt):: mvcnt
!  type(t_mvcnt):: mvcnt_save ! martin added for debug
!
...
! rigid-body move parameters
  ...
  RTYPE clurb_strbail,clurb_discrit
...
! constraint requests
  ...
  character(MAXSTRLEN) hs_seq_file
  logical do_frz,frz_report,skip_frz
  
  !Martin added
  RTYPE  selec_proba,selec_proba_neg,selec_proba_pos
  RTYPE  hsq_freq,hs_freq
  
  logical HSQ_in_progress,hsq_move
  logical HS_in_progress,hs_move
  
  
  integer, allocatable :: conjugate(:)
  
  integer hsq_steps,qs_freq
  integer alt_mode
