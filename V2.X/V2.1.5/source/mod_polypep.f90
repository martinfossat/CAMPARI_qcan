...
!
  type t_dpgrp
    ...
    integer nats,nc,cgn
    RTYPE qnm,tc
    RTYPE, ALLOCATABLE:: qnm_limits(:),tc_limits(:)
  end type t_dpgrp
  type t_fosgrp
    integer, ALLOCATABLE:: ats(:)
    RTYPE, ALLOCATABLE:: wts(:)
!#tyler pka:
    RTYPE, ALLOCATABLE:: wts_limits(:,:)
    !RTYPE val_limits(3,2) !always (3,2) but I only want it allocated if it is used
    !MArtin : changed to sva ebug
    logical is_prot
    integer nats
    RTYPE val(3)
    RTYPE val_limits(3,3)
  end type t_fosgrp
  type t_at
    integer, ALLOCATABLE:: bb(:),sc(:),pol(:)
    integer nbb,nsc,npol,ndpgrps,nfosgrps,na
    type (t_dpgrp), ALLOCATABLE:: dpgrp(:)
    type (t_fosgrp), ALLOCATABLE:: fosgrp(:)
  end type t_at
  
 
  RTYPE, ALLOCATABLE:: debug_energy(:,:),debug_energy_cumul(:,:)! Martin : delete once debugging is done 
