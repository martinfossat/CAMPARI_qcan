...
!
 ...
 &             1088,1116,1125,1100,1140,1159,1172,1074,   0,&
 &                0,1100,1088, 157, 157,1268,1279,1251,1268,&
 &                 1251,1279,1125,0,0,157/
 ...
 &             1089,1117,1126,1101,1141,1160,1173,1075,   0,&
 &                0,1101,1089,158,158,1269,1280,1252,1269,1252,&
 &                 1280,1126,0,0,158/
 ...
 &             1090,1118,1127,1102,1142,1161,1174,1076,   0,&
 &                0,1102,1090,159,159,1270,1281,1253,1270,1253,&
 &                1281,1127,0,0,159/
 ...
 &             1091,1119,1128,1103,1143,1162,1175,1077,   0,&
 &                0,1103,1091,160,160,1271,1282,1254,1271,1254,&
 &                1282,1128,0,0,160/
 ...
 &             1092,1120,1129,1104,1144,1163,1176,1078,   0,&
 &                0,1104,1092,161,161,1272,1283,1255,1272,1255,&
 &                1283,1129,0,0,161/
 ...
 &             1093,1121,1130,1105,1145,1164,1177,1079,   0,&
 &                0,1105,1093,162,162,1273,1284,1256,1273,1256,&
 &                1284,1130,0,0,162/
!
...
! some more small molecule stuff (mostly OPLS)
  ...
  an(127) = 136.00d0 ! CNH in Smith GDN
  an(128) = 112.00d0 ! HNH in Smith GDN
...
!#tyler pka
      ...
      at(i)%bb(at(i)%nbb) = n
      atmres(n) = rsmol(imol,1)
      call z_add(1215,bo(1),an(1),dh(1),0,0,0,0)
!#tyler pka
    else if (resname.eq.'CLX') then
      at(i)%nbb = at(i)%nbb + 1
      at(i)%bb(at(i)%nbb) = n
      atmres(n) = rsmol(imol,1)
      call z_add(1214,bo(1),an(1),dh(1),0,0,0,0)
!
...
!
      ...
      if (ua_model.eq.0) then
        ...
        at(i)%sc(at(i)%nsc) = n
        atmres(n) = rsmol(imol,1)
        call z_add(964,bo(2),an(3),chi(1,i),n-5,n-4,n-3,0)
        at(i)%nsc = at(i)%nsc + 1
        at(i)%sc(at(i)%nsc) = n
        atmres(n) = rsmol(imol,1)
        call z_add(964,bo(2),an(3),an(3),n-6,n-5,n-1,1)
        at(i)%nsc = at(i)%nsc + 1
        at(i)%sc(at(i)%nsc) = n
        atmres(n) = rsmol(imol,1)
        call z_add(964,bo(2),an(3),an(3),n-7,n-6,n-2,-1)
        chiline(2,i) = n
        nchi(i) = nchi(i) + 1
        at(i)%nsc = at(i)%nsc + 1
        at(i)%sc(at(i)%nsc) = n
...
!
 ...
 &           (resname.eq.'LYD').OR.(resname.eq.'T1P').OR.&
 &           (resname.eq.'S1P').OR.(resname.eq.'Y1P').OR.&
 &           (resname.eq.'GLX').OR.(resname.eq.'LYX').OR.&
 &           (resname.eq.'HDX').OR.(resname.eq.'T2P').OR.&
 &           (resname.eq.'S2P').OR.(resname.eq.'Y2P').OR.&
 &           (resname.eq.'ASX').OR.(resname.eq.'HEX').OR.&
 &           (resname.eq.'TYX').OR.(resname.eq.'TXP').OR.&
 &           (resname.eq.'SXP').OR.(resname.eq.'YXP')) then
...
!   generic amino acid residues
 ...
 &          (seqtyp(i).eq.51).OR.&
 &          ((seqtyp(i).ge.108).AND.(seqtyp(i).le.115)).OR.&
 &          (seqtyp(i).eq.116).OR.(seqtyp(i).ge.119)) then
!
!     backbone-N
      shf = 0
