...
!
  implicit none
!
  integer i,ii,rs,k,ipdb,atti,nrch,imol,ndump,kk,mt,rsshft,rsshft2
  integer attiwr,rswr,nunk
  ...
  RTYPE occup,shf,zero,ninety,outv(3)
!
 30    format (a6,i5,1x,a4,a1,a3,1x,a1,i4,4x,3f8.3,1x,f5.2,'  0.00')
...
!
  zero = 0.0
  ninety = 90.0
  if ((pdb_force_box.EQV..true.).AND.(bnd_type.eq.1).AND.(bnd_shape.ne.1).AND.(bnd_shape.ne.3)) then
...
!       overwrite some internal names for full pdb-compatibility
        ...
        tstr(1:6) = 'ATOM  '
        extra = ' '
        call name_heavies(ii,rs,ana,extra)
!
...
!       override the ATOM-string for hetero atoms
 ...
 &   (.NOT.((b_type(ii).ge.1074).AND.(b_type(ii).le.1186)))) then
            tstr(1:6) = 'HETATM'
         end if
        end if
!
!       write to file but watch for formatting errors (correct crudely)
        ...
        if ((pdb_force_box.EQV..true.).AND.(bnd_type.eq.1)) then
          ...
          write (ipdb,30) tstr,attiwr,ana,extra,resnameprt,chch,rswr,outv(1),outv(2),outv(3)+shf,occup
        else       
          write (ipdb,30) tstr,attiwr,ana,extra,resnameprt,chch,rswr,x(ii),y(ii),z(ii)+shf,occup
        end if
!
...
!             identify the biotype for C3*
 ...
 &                (b_type(kk).eq.626).OR.(b_type(kk).eq.938)) then
                  if ((pdb_force_box.EQV..true.).AND.(bnd_type.eq.1)) then
                    ...
                    write (ipdb,30) tstr,atti,ana,extra,resnameprt,chch,rs,outv(1),outv(2),outv(3)+shf,occup
                  else       
                    write (ipdb,30) tstr,atti,ana,extra,resnameprt,chch,rs,x(kk),y(kk),z(kk)+shf,occup
                  end if
                else
...
!
subroutine name_heavies(ii,rs,ana,extra)
!
  use iounit
  use atoms
  use pdb
...
!
 ...
 &         (b_type(ii).eq.368).OR.(b_type(ii).eq.553)) then
    ...
    if (pdb_convention(1).eq.3) ana(2:4) = 'NT2'
...
!   we'll modify the terminal one
 ...
 &         (b_type(ii).eq.634).OR.(b_type(ii).eq.946)) then
    if ((pdb_nucmode.eq.1).AND.(pdb_convention(1).eq.1)) then
      ana(1:1) = '2'
    end if
...
!
  ...
  else if ((b_type(ii).eq.102).OR.(b_type(ii).eq.110)) then
    if (ua_model.eq.0) then
      if (pdb_convention(1).eq.4) then
  ...
  else if ((b_type(ii).eq.125).OR.(b_type(ii).eq.1132)) then
    if (ua_model.eq.0) then
      if (pdb_convention(1).eq.4) then
        if (ii.eq.at(rs)%sc(10)) ana(1:1)='2'
        if (ii.eq.at(rs)%sc(11)) ana(1:1)='3'
 ...
 &               (b_type(ii).eq.197).OR.(b_type(ii).eq.1166)) then
    if (ua_model.eq.0) then
      if (pdb_convention(1).eq.4) then
        if (ii.eq.at(rs)%sc(8)) ana(1:1)='2'
        if (ii.eq.at(rs)%sc(9)) ana(1:1)='3'
 ...
 &         (b_type(ii).eq.1083)) then
  ...
  else if ((b_type(ii).eq.278).OR.(b_type(ii).eq.292).OR.(b_type(ii).eq.1111)) then
    if (ua_model.eq.0) then
      if (pdb_convention(1).eq.4) then
        if (ii.eq.at(rs)%sc(11)) ana(1:1)='2'
        if (ii.eq.at(rs)%sc(12)) ana(1:1)='3'
  ...
  else if ((b_type(ii).eq.280).OR.(b_type(ii).eq.294).OR.(b_type(ii).eq.1113)) then
    if (ua_model.eq.0) then
      if (pdb_convention(1).eq.4) then
        if (ii.eq.at(rs)%sc(13)) ana(1:1)='2'
        if (ii.eq.at(rs)%sc(14)) ana(1:1)='3'
    ...
    if (pdb_convention(1).eq.3) ana(3:4) = 'T2'
...
!
  implicit none
!
  integer xdr,ndump,magic,ret,natsel,istp,i,j,k,kk,mt,imol
! note that the XDR-code only works with single precision variables (float in C)
  real(KIND=4) box(3,3),xtcprec,shf
...
! otherwise VMD plugin can't read it properly
  if (use_trajidx.EQV..true.) then
   ...
   &in FMSMC_prtxtc(...). This is an omission bug.'
        ...
        coords(3*i-2) = x(pdboutlst(i))/10.0
        coords(3*i-1) = y(pdboutlst(i))/10.0
        coords(3*i) = (z(pdboutlst(i))+shf)/10.0
      end do
    end if
  else
    j = 0
   ...
   &in FMSMC_prtxtc(...). This is an omission bug.'
        ...
        do i=atmol(imol,1),atmol(imol,2)
          j = j + 1
          mt = moltypid(imol)
          coords(3*j-2) = x(i)/10.0
          coords(3*j-1) = y(i)/10.0
          coords(3*j) = (z(i)+shf)/10.0
        end do
      end do
    end if
  end if
...
!   write header
 ...
 &' on '//rc(1)(5:6)//'/'//rc(1)(7:8)//'/'//rc(1)(1:4)//'.'
    ...
    dcdhs(1) = (nsim-nequil)/xyzout ! final number of snapshots
    dcdhs(2) = nequil+xyzout-mod(nequil,xyzout) ! first snapshot
...
!
  if (use_trajidx.EQV..true.) then
   ...
   &in FMSMC_prtdcd(...). This is an omission bug.'
        ...
        coords(i) = x(pdboutlst(i))
        coords(pdbeffn+i) = y(pdboutlst(i))
        coords(2*pdbeffn+i) = (z(pdboutlst(i))+shf)
      end do
    end if
  else
    j = 0
...
!
  implicit none
!
  integer ndump,i,j,mt,imol,ncid,intin,dims(3),k,kk,framei
!  integer nf90_redef,nf90_put_att,nf90_def_dim
! note that the XDR-code only works with single precision variables (float in C)
  real(KIND=4) shf,timepos
  real(KIND=4), ALLOCATABLE:: coords(:)
  RTYPE box(3,3)
  logical ismember,dohead
...
!
  integer iint,freeunit,t1,t2,t3,t4,m1
...
!
  ...
  if ((use_REMC.EQV..true.).OR.((use_MPIAVG.EQV..true.).AND.(force_singlexyz.EQV..true.))) then
    itn(1:bleng+6) = 'N_'//nod(1:tl)//'_'//basename(1:bleng)
    t2 = bleng + 6
  else
    itn(1:bleng) = basename(1:bleng)
    t2 = bleng
  end if
  itn2(1:bleng+6) = 'N_'//nod(1:tl)//'_'//basename(1:bleng)
  t3 = bleng+6
#else
  itn(1:bleng) = basename(1:bleng)
...
!
 ...
 &in tinkerxyz(...). This is an omission bug.'
      ...
      if ((pdb_force_box.EQV..true.).AND.(bnd_type.eq.1)) then
        ...
        write (iu,30) j,atnam(i),x(i),y(i),z(i)+shf,attyp(i),(i12(k,i)-offset,k=1,n12(i))
      end if
    end do
  end do
!
