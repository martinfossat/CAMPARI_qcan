...
!
  integer k,catnrs
  RTYPE veci(calcsz),vecj(calcsz),vec1(calcsz),vec2(calcsz),vec3(calcsz),val,tvec(3),qrot(4),cto(3),ctn(3),vectmp(3)
...
!
  ...
  integer frlist(lilen),tmplist(it%nmbrs)
!
  jcnt = 1
  icnt = 1
  ncnt = 0
...
!
 44 format('Quality of clustering:',/,'Tightness: ',g12.5,/,'Singles  : ',g12.5,/,'Entropy  : ',g12.5,/,'Total    : ',g12.5,/)
  if (normer.gt.0) then
  ...
  write(ilog,44) quals(:),sum(quals)/3.0
!
end
!
!-------------------------------------------------------------------------------
...
!    allocate(cto%fewtsnb(cfrom%nbalsz,2))
...
!
  ...
  RTYPE, ALLOCATABLE:: bla1(:,:),bla2(:),bla3(:,:),bla4(:),bla5(:,:),bla6(:),bla7(:,:),bla8(:)
  character(1) cu,cvt
...
! for options 4, 9, and 10, create an average static weight from the local weights and scale data
  ...
  if (cdis_crit.eq.4) then
    ...
    bla8(:) = sqrt(normer*bla8(:))
    do i=1,sz3
      bla7(i,1:sz2) = cludata(i,1:sz2)*bla8(i)
    end do
...
! now use eigenvectors as per user request
  if (pcamode.eq.3) then ! transform, write both, potentially reduce dimensionality for subsequent clustering
    deallocate(bla5)
...
!
    ...
    if (use_REMC.EQV..true.) then
      fn = 'N_'//nod(1:tl)//'_PRINCIPAL_COMPONENTS.dat'
    else
...
!
 ...
 66 format(100000(1x,g10.4))
!
    write(ipca,67) bla2(1:min(effsz1,sz2))
    write(ipca,67) bla6(1:min(effsz1,sz2))
    do i=1,sz2
      write(ipca,66) bla5(i,1:effsz1)
    end do
    close(unit=ipca)
!
...
!
  ...
  call int2str(myrank,nod,tl)
  if (use_REMC.EQV..true.) then
    fn = 'N_'//nod(1:tl)//'_PRINCIPAL_COMPONENTS.evs'
  else
...
!
  write(ipca,67) bla2(1:min(effsz1,sz2))
  if (pcamode.eq.3) write(ipca,67) bla6(1:min(effsz1,sz2))
  do i=1,effsz1
    write(ipca,66) bla1(i,1:effsz1)
  end do
  close(unit=ipca)
!
  deallocate(bla7)
...
!
  ...
  if (use_REMC.EQV..true.) then
    ...
    t2 = bleng + 6
    fn = 'N_'//nod(1:tl)//'_STRUCT_CLUSTERING.vmd'
  else
...
!
  ...
  if (pdb_analyze.EQV..false.) then
    ...
    if (pdb_fileformat.eq.2) then
      ...
      if ((tl.ge.t4).AND.(tl.lt.t5)) then
        fn =  fn2(t4:tl)//'N_'//nod(1:j)//'_'//fn2((tl+1):t5)
      else
        fn =  'N_'//nod(1:j)//'_'//fn2(t4:t5)
      end if
