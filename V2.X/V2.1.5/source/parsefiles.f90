...
!-----------------------------------------------------------------------
!
subroutine read_HS_residues() !Martin added
!
  use iounit
  use atoms
  use interfaces
...
!
 ...
 79   format(FORM_MAXSTRLEN)
  ...
  iunit = freeunit()
    ...
    open (unit=iunit,file=hs_seq_file,status='old')
  do while(.true.)
     line=line+1
    read(iunit,79,iostat=iomess) str2 ! Martin : reads the line 
    
    ! Martin : from there reads file, checks the status (invalid or end of file) 
    if (iomess.eq.-1) then
      line=line-1
      exit
...
subroutine read_charge_states() !Martin added
!
  use iounit
  use atoms
  use interfaces
 ...
 79   format(FORM_MAXSTRLEN)
  ...
  open (unit=iunit,file=charge_state_file,status='old')
  do while(.true.)
     line=line+1
    read(iunit,79,iostat=iomess) str2 ! Martin : reads the line 
 ...
 &input in read_atmidx(...).'
    ...
    allocate(q_states(line,n_res))
    n_charge_states=line
    line=0
 ...
 &input in read_atmidx(...).'
  ...
  allocate(acc%HSQ_state_tries(n_charge_states,n_charge_states))
  allocate(acc%HSQ_state_acc(n_charge_states,n_charge_states))
  allocate(acc%curr_tries(2))
  
  acc%HSQ_state_tries(:,:)=0
  acc%HSQ_state_acc(:,:)=0
...
!
subroutine read_atmpairidx(iunit,nlst,tmpvec,tvsz,lsort,checkbounds)
!
  use iounit
  use atoms
  use interfaces
...
!
  ...
  integer, ALLOCATABLE:: iv1(:,:)
  logical lsort,checkbounds,notdone,atrue
...
!
 ...
 &read_atompairidx(...). Check input files.'
      call fexit()
    end if
!
    nlst = nlst + 1
    if (checkbounds.EQV..true.) then
      if (nlst.gt.n*(n-1)/2) then
        write(ilog,*) 'Fatal. Set list provided to read_atmpairidx(...) is longer &
 &than the number of unique atom pairs in the system. This is fatal.'
        call fexit()
      end if
    end if
    if (nlst.gt.tvsz) then
...
!
  implicit none
!
  integer i,j,k,rs,ttc,ttc2,iunit,freeunit,t1,t2,kk,k1,k2,modstep,maxstp,minstp
  logical exists,atrue
  RTYPE normer,random
...
!
 ...
 &for Cartesian coordinate-based, structural clustering.'
    ...
    if (use_MPIAVG.EQV..true.) then
      if (myrank.eq.masterrank) allocate(cludata(calcsz,mpi_nodes*cmaxsnaps))
    else
 ...
 &                     cdofset(k,2),bio_code(b_type(cdofset(k,2))),atmres(cdofset(k,2)),amino(seqtyp(atmres(cdofset(k,2))))
      ...
      if (cdis_crit.eq.8) then
        ...
        normer = 1.0/sum(cl_imvec(:))
        cl_imvec(:) = sqrt(normer*cl_imvec(:))
      end if
    else
      do i=1,ttc
        cdofset(i,1) = i
...
! get the unique, sorted set of indices
  if ((cdis_crit.ge.7).AND.(cdis_crit.le.9)) then
    kk = min(1000000,n*(n-1)/2)
    allocate(tmpvec2(kk,2))
    calcsz = 0
    call read_atmpairidx(iunit,calcsz,tmpvec2,kk,atrue,atrue)
!
    if (calcsz.le.0) then
...
!
  implicit none
!
  integer k,t1,t2,iunit,i,j,freeunit
  integer, ALLOCATABLE:: tmpvec(:)
...
!
  allocate(tmpvec(nsim))
!
  call read_atmidx(iunit,ntbrks,tmpvec,k,atrue,afalse)
!
  k = ntbrks
  do i=1,k
...
!
 ...
 &than the specified simulation length. This is fatal.'
      call fexit()
    end if
!
    tmpvec(nlst,3) = nlst
...
!     assume weight of unity
      ...
      if ((iomess.eq.-1).OR.(iomess.eq.1)) then
        tmpvec2(nlst) = 1.0
      else
        tmpvec2(nlst) = fpv
...
!
  ...
  if (buff.le.0) then
    write(ilog,*) 'Fatal. Setting for simulation length (',nsim,') and selected frames &
...
!
  deallocate(tmpvec)
  deallocate(tmpvec2)
  if (nsim.gt.framelst(framecnt)) then
...
!
  implicit none
!
  integer iunit,freeunit,iomess,t1,t2,i,j,ii,k,dc,lastin,tl
  character(2) rsnma
  character(3) resname
...
!
 ...
 &input (',torfile(t1:t2),'). Turning off torsional term.'
    use_TOR = .false.
    scale_TOR = 0.0
    close(unit=iunit)
    deallocate(par_TOR2)
...
!   for both std. harmonic and exponential torsional potentials there are maximally twelve parameters
!   six targets, and six spring constants / widths
!   for both versions as starting configuration bias potentials, there are only the six
!   spring constants / widths
!
...
!
 ...
 &nal input (',torfile(t1:t2),'). Turning off torsional term.'
        use_TOR = .false.
        scale_TOR = 0.0
        close(unit=iunit)
        deallocate(par_TOR2)
...
!
      ...
      if ((k.gt.nseq).OR.(k.le.0)) then
        write(ilog,*) 'Warning. Requested torsional potential for re&
...
!
 ...
 &ional input (',torfile(t1:t2),'). Turning off torsional term.'
          use_TOR = .false.
          scale_TOR = 0.0
          close(unit=iunit)
          deallocate(par_TOR2)
...
!     add the sugar
 ...
 & have more than six nucleic acid d.o.f.s. This is most certainly an omission bug.'
      ...
      if (6+nchi(i).lt.MAXTORRES) par_TOR(i,MAXTORRES+6+nchi(i)+1:2*MAXTORRES) = 0.0
  ...
  if (j.eq.0) then
    write(ilog,*) 
    write(ilog,*) 'WARNING: No torsional potentials requested (?). D&
 &isabling term.'
    write(ilog,*)
    use_TOR = .false.
    scale_TOR = 0.0
...
!
  if (tor_report.EQV..true.) then
    tl = 2
    write(ilog,*)
    write(ilog,*) '--- Summary of Torsional Terms in the System ---'
    write(ilog,*)
    do i=1,nseq
      stringy(:) = '              '
...
! UNLESS mr.user elected to use the default -> check again
  ...
  if (exists.EQV..false.) then
    write(ilog,*) 'WARNING: Cannot open spec.d input file (',&
 &polfile(t1:t2),') for FMCSC_POLFILE. Turning off polymeric terms.'
    use_POLY = .false.
    scale_POLY = 0.0
    return
  end if
...
!
 ...
 &index has to be either 0 or 1. Ignoring entire request for residue ',mn,'.'
    ...
    rsncp(mn)%alsz = mnrt
    call get_reals_from_str(str3,at(mn)%na,rsncp(mn)%dis(:),ii,rsncp(mn)%nbs)
...
!
  close(unit=iunit)
!
  if (nlst.gt.0) bonded_patched = .true.
!
end
!
!-----------------------------------------------------------------------
...
!
 ...
 &mnwt,') in applying a FOS patch. Request will be ignored (most likely fatal later on) ...'
    ...
    readrfos = 0
    lk = 3
    call get_reals_from_str(str3,lk,mnrfos(1:3),i,readrfos)
!
    nlst = nlst + 1
    if (fdvec(mn).EQV..false.) then
      tmpmat(nlst,1) = mn
...
!       this has to be the first mol. of its type
 ...
 &rrespond to the first mol. of their type in sequence:'
...
!       both mol.s have to be the first of their respective type
 ...
 &            (jmol.ne.moltyp(moltypid(jmol),1))) then
            write(ilog,*) 'Atoms for cross-intermolecular terms have&
...
!       the first mol has to be the first of its type, the second the second
          ...
          if (imol.ne.moltyp(moltypid(imol),1)) then
            write(ilog,*) 'Atoms for self-intermolecular terms have &
 &to correspond to the first two mol. of their type in sequence:'
            write(ilog,76) dum(1),imol,moltyp(moltypid(imol),1)
            cycle
          end if
          do kmol=1,nmol
...
!     intermolecular type-wise requests
 ...
 &          (typ2.le.0).OR.(typ2.gt.nmoltyp)) then
          write(ilog,*) 'Range exception occurred while processing G&
 &PC input:'
        ...
        if (typ1.eq.typ2) then
          if (moltyp(typ1,2).eq.1) then
            write(ilog,*) 'Intermolecular term for molecule type ',&
 &typ1,' is ignored, as there is only one molecule of this type.'
            cycle
          end if
          do kmol=1,nmol
            if ((moltypid(kmol).eq.typ1)&
 &             .AND.(kmol.ne.moltyp(typ1,1))) exit
          end do
...
!         fully there, though!)
          do ii=1,nm1
            ...
            if (iomess.eq.-1) then
              write(ilog,*) 'Warning. Incomplete matrix found while &
 &processing GPC input:'
              write(ilog,74) abc(1:1),typ1,typ2
              havetogo = .true.
              exit
            else if (iomess.eq.2) then
              write(ilog,*) 'Fatal. I/O error while processing GPC i&
 &nput.'
              call fexit()
            end if
          end do
...
!       of it is used (has to be fully there, though!)
        do ii=1,nm1
          read(iunit,*,iostat=iomess) (dummat(ii,jj),jj=1,nm1)
          if (iomess.eq.-1) then
            write(ilog,*) 'Warning. Incomplete matrix found while pr&
 &ocessing GPC input:'
            write(ilog,73) abc(1:1),typ1
            havetogo = .true.
            exit
          else if (iomess.eq.2) then
            write(ilog,*) 'Fatal. I/O error while processing GPC inp&
 &ut.'
            call fexit()
          end if
        end do
...
!         fully there, though!)
          ...
          do ii=1,nm1
            do jj=ii,nm1
              if (dummat(ii,jj).gt.0) then
                gpc%nlst = gpc%nlst + 1
                gpc%lst(gpc%nlst,1) = atmol(moltyp(typ1,1),1)+ii-1
          ...
          do ii=1,nm1
            do jj=1,nm2
              if (dummat(ii,jj).gt.0) then
                gpc%nlst = gpc%nlst + 1
                gpc%lst(gpc%nlst,1) = atmol(moltyp(typ1,1),1)+ii-1
                gpc%lst(gpc%nlst,2) = atmol(moltyp(typ2,1),1)+jj-1
                gpc%lst(gpc%nlst,3) = dummat(ii,jj)
                gpc%lty(gpc%nlst) = .true.
              end if
            end do
        ...
        do ii=1,nm1
          do jj=ii+1,nm1
            if (dummat(ii,jj).gt.0) then
              gpc%nlst = gpc%nlst + 1
              gpc%lst(gpc%nlst,1) = atmol(moltyp(typ1,1),1)+ii-1
              gpc%lst(gpc%nlst,2) = atmol(moltyp(typ1,1),1)+jj-1
              gpc%lst(gpc%nlst,3) = dummat(ii,jj)
              gpc%lty(gpc%nlst) = .true.
            end if
          end do
...
!        write(*,*) rs1,rs1,'start ',tbp%rsmat(rs1,rs1),' and end'
! &,tbp%rsvec(rs1)
!    end if
!    do rs2=rs1+1,nseq
!      if (tbp%rsmat(rs1,rs2).gt.0) then
!        write(*,*) rs1,rs2,'start ',tbp%rsmat(rs1,rs2),' and end'
! &,tbp%rsmat(rs2,rs1)
!      end if
!      if (((tbp%rsmat(rs1,rs2).gt.0).AND.(tbp%rsmat(rs2,rs1).eq.0))
! &.OR.((tbp%rsmat(rs1,rs2).eq.0).AND.(tbp%rsmat(rs2,rs1).gt.0)))then
...
!
   ...
   &atom ',rs,' in ',frzfile(t1:t2),': ',dofs,' - ignored.'
        ...
        if (dofs.eq.'INT') then
          ...
          if ((izrot(rs)%alsz.le.0).OR.(rs.eq.atmol(imol,1))) then 
            write(ilog,*) 'Warning. Requested dihedral angle constraint request for Z-matrix line ',rs,&
 &', which does not correspond to a rotatable dihedral angle. Ignored.'
            ...
            write(ilog,*) 'Fatal. Requested dihedral angle constraint request for Z-matrix line ',rs,&
...
!
 ...
 &volume. Hard-sphere potentials will never work for FEG.'
    call fexit()
  else if (nindx.ne.12) then
    write(ilog,*) 'Fatal. Ghosting requires the IPP-exponent to be e&
 &xactly 12.'
    call fexit()
    
  else if (use_IMPSOLV.EQV..true.) then ! Martin : added for moses
        write(ilog,*) 'Using a ghost potential with free energy fo solvation potential on. This was added by Martin, and can not &
        &be done in the original CAMPARI'
  !else if ((use_IMPSOLV.EQV..true.).OR.(use_WCA.EQV..true.).OR.&
  else if ((use_WCA.EQV..true.).OR.&
 &         (use_TABUL.EQV..true.).OR.(use_DSSP.EQV..true.).OR.&
 &         (use_ZSEC.EQV..true.).OR.(use_TOR.EQV..true.).OR.&
 &         (use_DREST.EQV..true.).OR.(use_POLY.EQV..true.).OR.(use_CORR.EQV..true.)) then
    write(ilog,*) 'Fatal. Ghosting is not supported for the current &
 &full Hamiltonian (unintentionally left an energy term active?).'
    call fexit()
!  else if ((use_CORR.EQV..true.).AND.(fegmode.eq.2)) then
!    write(ilog,*) 'Fatal. Ghosting in mode 2 is not supported if int&
! &ra-residue correction terms (EXTRA) are turned on.'
!    call fexit()
  end if
!
...
!
!-----------------------------------------------------------------------
!#Replica exchange file  reading part ! #martin
...
!
  do i=1,re_conddim! Loops through the number of dimenson of the remc.in file
    do k=i+1,re_conddim
      if (re_types(i).eq.re_types(k)) then! Check if any two variable in the remc file are the same
...
#endif
! #tyler pka
    else if (re_types(k).eq.35) then
      par_PKA(1)=re_mat(myrank+1,k)
      do_pka = .true.
      do_pka_2 = .true.
      do_pka_3 = .true.
      write(ilog,*) 'Using the charge down dimension for replica exchange.'
      if ((par_PKA(1).lt.0.0).OR.&
 &            (par_PKA(1).gt.1.0)) then
        write(ilog,*) 'Fatal. Requested illegal value for pka scal&
 &ing of charges in REMC.'
        call fexit()
      end if
      
    else if (re_types(k).eq.36) then
      par_PKA(2)=re_mat(myrank+1,k)
      do_pka = .true.
      do_pka_2 = .true.
      do_pka_3 = .true.
      use_softcore=.true.
      write(ilog,*) 'Using the LJ dimension for replica exchange. &
      &Softcore potential use turned on.'
      if ((par_PKA(2).lt.0.0).OR.&
 &            (par_PKA(2).gt.1.0)) then
        write(ilog,*) 'Fatal. Requested illegal value for pka scal&
 &ing of sigmas in REMC.'
        call fexit()
      end if
      
      else if (re_types(k).eq.37) then ! Martin added
      par_PKA(3)=re_mat(myrank+1,k)
      do_pka = .true.
      do_pka_2 = .true.
      do_pka_3 = .true.
      write(ilog,*) 'Using the FOS dimension for replica exchange.'
      if ((par_PKA(3).lt.0.0).OR.&
 &            (par_PKA(3).gt.1.0)) then
        write(ilog,*) 'Fatal. Requested illegal value for pka scal&
 &ing of FOS in REMC.'
        call fexit()
      end if
      
      else if (re_types(k).eq.38) then ! Martin added
      par_PKA(4)=re_mat(myrank+1,k)
      do_pka = .true.
      do_pka_2 = .true.
      do_pka_3 = .true.
      write(ilog,*) 'Using the charge up dimension for replica exchange.'
      if ((par_PKA(4).lt.0.0).OR.&
 &            (par_PKA(4).gt.1.0)) then
        write(ilog,*) 'Fatal. Requested illegal value for pka scal&
 &ing of FOS in REMC.'
        call fexit()
      end if
      
      else if (re_types(k).eq.39) then ! Martin added for moses
      scale_FEGS(4) = re_mat(myrank+1,k)
      use_FEGS_FOS = .true.!because use_FEGS(4) was already taken for unknown reason)
      if (scale_FEGS(4).lt.0.0) then
        write(ilog,*) 'Fatal. Requested illegal value for FEG_IPP in&
 & REMC.'
        call fexit()
      end if
