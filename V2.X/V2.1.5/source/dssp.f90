...
!-----------------------------------------------------------------------
!
subroutine assign_bridge(ps1,ps2)
!
  use iounit
  use polypep
  use atoms
...
!
  do i=2,pep_sz-2
    if (molofrs(rspep(i-1)).eq.molofrs(rspep(i+2))) then
...
!
  if (use_frame_weights.EQV..false.) then
    ...
    if (use_REMC.EQV..true.) then
      ...
      fn =  'N_'//xpont(1:lext2)//'_DSSP_RES.dat'
...
!
  ...
  if (use_REMC.EQV..true.) then
    call int2str(myrank,xpont,lext2)
    fn =  'N_'//xpont(1:lext2)//'_DSSP_NORM_RES.dat'
  else if (use_MPIAVG.EQV..true.) then
...
!
  ...
  if (use_REMC.EQV..true.) then
    call int2str(myrank,xpont,lext2)
    fn =  'N_'//xpont(1:lext2)//'_DSSP_NORM.dat'
  else if (use_MPIAVG.EQV..true.) then
    fn = 'DSSP_NORM.dat'
  end if
#else
...
!
  ...
  if (use_REMC.EQV..true.) then
    call int2str(myrank,xpont,lext2)
    fn =  'N_'//xpont(1:lext2)//'_DSSP_HIST.dat'
  else if (use_MPIAVG.EQV..true.) then
    fn = 'DSSP_HIST.dat'
  end if
#else
...
!
  ...
  if (use_REMC.EQV..true.) then
    call int2str(myrank,xpont,lext2)
    fn =  'N_'//xpont(1:lext2)//'_DSSP_EH_HIST.dat'
  else if (use_MPIAVG.EQV..true.) then
    fn = 'DSSP_EH_HIST.dat'
  end if
#else
