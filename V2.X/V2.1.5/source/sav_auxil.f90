...
!
  ...
  integer i,k,ii,kk,rs,rs2,bblst(maxval(at(1:nseq)%nbb+at(1:nseq)%nsc)+50),bbnr,aone
  integer ptlst(20),ptnr,sclst(maxval(at(1:nseq)%nbb+at(1:nseq)%nsc)),scnr,k_12
  RTYPE dis,maxd,totsav,rkk,rii,dis2,svec(3),vs(50)
...
! Martin : again this has to be done so that the geometric length and the registered bond length for the limits match
 ...
 &                   (z(ii) - z(kk))**2)
          ...
          if (dis.lt.maxd) then
            if (dis.gt.(maxd-2*atr(kk))) then
              atsavmaxfr(ii) = atsavmaxfr(ii) -&
 &                        ((maxd-dis)/(2.0*atr(kk)))*rkk*atvol(kk)
            else if (dis.lt.(atr(ii)+atr(kk))) then
             if (atr(ii).ge.atr(kk)) then
                if (dis.gt.(atr(ii)-atr(kk))) then
                  atsavmaxfr(ii) = atsavmaxfr(ii) -&
 &         ((dis-(atr(ii)-atr(kk)))/(2.0*atr(kk)))*rkk*atvol(kk)
                end if
              else
                if (dis.gt.(atr(kk)-atr(ii))) then
                  atsavmaxfr(ii) = atsavmaxfr(ii) -&
 &         (rkk*atvol(kk) -&
 &          ((atr(kk)+atr(ii)-dis)/(2.0*atr(ii)))*rii*atvol(ii))
                else
                  atsavmaxfr(ii) = atsavmaxfr(ii) -&
 &                               (rkk*atvol(kk) - rii*atvol(ii))
                end if
              end if
            else
              atsavmaxfr(ii) = atsavmaxfr(ii) - rkk*atvol(kk)
            end if
            if (dis.gt.(maxd-2*atr(ii))) then
              atsavmaxfr(kk) = atsavmaxfr(kk) - &
 &                        ((maxd-dis)/(2.0*atr(ii)))*rii*atvol(ii)
            else if (dis.lt.(atr(ii)+atr(kk))) then
              if (atr(kk).ge.atr(ii)) then
                if (dis.gt.(atr(kk)-atr(ii))) then
                  atsavmaxfr(kk) = atsavmaxfr(kk) -&
 &         ((dis-(atr(kk)-atr(ii)))/(2.0*atr(ii)))*rii*atvol(ii)
                end if
              else
                if (dis.gt.(atr(ii)-atr(kk))) then
                  atsavmaxfr(kk) = atsavmaxfr(kk) -&
 &         (rii*atvol(ii) -&
 &          ((atr(ii)+atr(kk)-dis)/(2.0*atr(kk)))*rkk*atvol(kk))
                else
                  atsavmaxfr(kk) = atsavmaxfr(kk) -&
 &                               (rii*atvol(ii) - rkk*atvol(kk))
                end if
              end if
            else
              atsavmaxfr(kk) = atsavmaxfr(kk) - rii*atvol(ii)
            end if
         end if
        end do
        do k=1,ptnr
...
! solvent isn't actually used
    ...
    do i=1,n
      atsav(i) = atstv(i) ! Martin : This should reinitialize of to atsavmaxfr*atvol ?                
      ! MArtin for now I think the atstv is a way to save the maximum volume and reinitialize every step
    end do  !It could or could not be 
    do rs=1,nseq
!     within-residue relevant interactions
 ...
 &           + (z(ii) - z(kk))**2
        ...
        if (dis.lt.maxd) then
          if (dis.gt.(maxd-2*atr(kk))) then
            atsav(ii) = atsav(ii) - ((maxd-dis)/(2.0*atr(kk)))*&
 &                                  rkk*atvol(kk)
 
          else if (dis.lt.(atr(ii)+atr(kk))) then
            atsav(ii) = atsav(ii) - (dis/(atr(ii)+atr(kk)))*&
 &                                  rkk*atvol(kk)
          else
          ...
          if (dis.gt.(maxd-2*atr(ii))) then
            atsav(kk) = atsav(kk) - ((maxd-dis)/(2.0*atr(ii)))*&
 &                                  rii*atvol(ii)
 
          else if (dis.lt.(atr(ii)+atr(kk))) then
            atsav(kk) = atsav(kk) - (dis/(atr(kk)+atr(ii)))*&
 &                                  rii*atvol(ii)
          else
...
!      end if 
 ...
 &                 (z(ii) - z(kk))**2)
        ...
        if (dis.lt.maxd) then
          if (dis.gt.(maxd-2*atr(kk))) then
            atstv(ii) = atstv(ii) - ((maxd-dis)/(2.0*atr(kk)))*&
 &                                                rkk*atvol(kk)
          else if (dis.lt.(atr(ii)+atr(kk))) then
            if (atr(ii).ge.atr(kk)) then
              if (dis.gt.(atr(ii)-atr(kk))) then
 ...
 &                           (rkk*atvol(kk) - rii*atvol(ii))
            ...
            atstv(ii) = atstv(ii) - rkk*atvol(kk)
          end if
          if (dis.gt.(maxd-2*atr(ii))) then
            atstv(kk) = atstv(kk) - ((maxd-dis)/(2.0*atr(ii)))*&
 &                                                rii*atvol(ii)
          else if (dis.lt.(atr(ii)+atr(kk))) then
            if (atr(kk).ge.atr(ii)) then
              if (dis.gt.(atr(kk)-atr(ii))) then
 ...
 &                           (rii*atvol(ii) - rkk*atvol(kk))
            ...
            atstv(kk) = atstv(kk) - rii*atvol(ii)
          end if
        end if
      end if
    end do
...
!   do nothing
 ...
 &      (resname.eq.'ASX').OR.(resname.eq.'LYX').OR.&
 ...
 &         (resname.eq.'UNK').OR.(resname.eq.'O2 ').OR.&
...
!   only if a base is present we need to add an extra H1*
 ...
 &sidue ',resname,' in pos. ',rs,' at the moment. Check back later(a1).'
      call fexit()
    end if
  end if
!
...
!   do nothing 
 ...
 &      (resname2.eq.'LYD').OR.(resname2.eq.'Y1P').OR.&
 &      (resname2.eq.'S1P').OR.(resname2.eq.'T1P').OR.&
 &      (resname2.eq.'PRO').OR.(resname2.eq.'Y2P').OR.&
 &      (resname2.eq.'S2P').OR.(resname2.eq.'T2P').OR.&
 &      (resname2.eq.'ORN').OR.(resname2.eq.'DAB').OR.&
 &      (resname2.eq.'HIP').OR.(resname2.eq.'GLX').OR.&
 &      (resname2.eq.'LYX').OR.(resname2.eq.'HDX').OR.&
 &      (resname2.eq.'HEX').OR.(resname2.eq.'ASX').OR.&
 &      (resname2.eq.'TYX').OR.(resname2.eq.'TXP').OR.&
 &      (resname2.eq.'SXP').OR.(resname2.eq.'YXP').OR.&
 &      (resname2.eq.'HIX')) then
    refh = at(rs-1)%sc(1)
...
!   do nothing
 ...
 &support residue ',resname2,' in pos ',rs-1,' . Check back later(a2).'
      call fexit()
    end if
  end if
  do i=1,ptnr
...
!   do nothing
 ...
 &      (resname.eq.'S1P').OR.(resname.eq.'T1P').OR.&
 &      (resname.eq.'HIP').OR.(resname.eq.'GLX').OR.&
 &      (resname.eq.'LYX').OR.(resname.eq.'HEX').OR.&
 &      (resname.eq.'HDX').OR.(resname.eq.'Y2P').OR.&
 &      (resname.eq.'S2P').OR.(resname.eq.'T2P').OR.&
 &      (resname.eq.'ASX').OR.(resname.eq.'YXP').OR.&
 &      (resname.eq.'SXP').OR.(resname.eq.'TXP').OR.&
 &      (resname.eq.'TYX').OR.(resname.eq.'HIX')) then
    bblst(1) = ni(rs)
    bblst(2) = hni(rs)
    bblst(3) = cai(rs)
...
!   (if possible) that the CH-unit is considered part of a peptide model compound
 ...
 &      (resname2.eq.'HIP').OR.(resname2.eq.'GLX').OR.&
 &      (resname2.eq.'LYX').OR.(resname2.eq.'HDX').OR.&
 &      (resname2.eq.'HEX').OR.(resname2.eq.'TXP').OR.&
 &      (resname2.eq.'YXP').OR.(resname2.eq.'SXP').OR.&
 &      (resname2.eq.'TYX').OR.(resname2.eq.'HIX').OR.&
 &      (resname2.eq.'ASX')) then
    bblst(1+bbnr) = ci(rs-1)
    bblst(2+bbnr) = oi(rs-1)
    bbnr = bbnr + 2
    if ((rs-1).eq.rsmol(molofrs(rs),1)) then
...
!
  implicit none
!
  integer i,rs,ptlst(20),ptnr,sclst(20),scnr,off,shf
  RTYPE vs(50)
  character(3) resname
!
...
!     setup partner atoms (including dummies to reproduce model compound)
 ...
 &              ,at(rs)%sc(5),vs(2),1)
  ...
  else if ((resname.eq.'S1P').OR.(resname.eq.'S2P').OR.(resname.eq.'SXP')) then
    do i=2-shf,at(rs)%nsc
      sclst(i-1+shf) = at(rs)%sc(i)
    end do
    scnr = at(rs)%nsc-1+shf
...
!     setup partner atoms (including dummies to reproduce model compound)
 ...
 &              ,at(rs)%sc(3),vs(2),1)
  ...
  else if ((resname.eq.'T1P').OR.(resname.eq.'T2P').OR.(resname.eq.'TXP')) then
    do i=2-shf,at(rs)%nsc
      sclst(i-1+shf) = at(rs)%sc(i)
    end do
    scnr = at(rs)%nsc-1+shf
...
!     setup partner atoms (including dummies to reproduce model compound)
 ...
 &              ,at(rs)%sc(8),vs(2),1)
  ...
  else if ((resname.eq.'ASP').OR.(resname.eq.'ASH').OR.(resname.eq.'ASX')) then
    do i=2-shf,at(rs)%nsc
      sclst(i-1+shf) = at(rs)%sc(i)
    end do
    scnr = at(rs)%nsc-1+shf
...
!     setup partner atoms (including dummies to reproduce model compound)
 ...
 &              ,at(rs)%sc(7),vs(2),1)
  ...
  else if ((resname.eq.'GLU').OR.(resname.eq.'GLH').OR.(resname.eq.'GLX')) then
!   WARNING: this is one of the sidechains we would actually have to sample
!            to account for the changing effect of the terminal CH3 on 
...
!     setup partner atoms (including dummies to reproduce model compound)
 ...
 &              ,at(rs)%sc(8),vs(2),1)
  ...
  else if ((resname.eq.'LYS').OR.(resname.eq.'LYD').OR.(resname.eq.'LYX')) then
!   WARNING: this is one of the sidechains we would actually have to sample
...
!     setup partner atoms (including dummies to reproduce model compound)
 ...
 &              ,at(rs)%sc(10),vs(2),1)
    ...
    else if ((resname.eq.'TYR').OR.(resname.eq.'TYO').OR.(resname.eq.'TYX')) then
    do i=2-shf,at(rs)%nsc
      sclst(i-1+shf) = at(rs)%sc(i)
    end do
    scnr = at(rs)%nsc-1+shf
...
!     setup partner atoms (including dummies to reproduce model compound)
 ...
 &                          vs(2),at(rs)%sc(11),vs(2),1)
  ...
  else if ((resname.eq.'Y1P').OR.(resname.eq.'Y2P').OR.(resname.eq.'YXP')) then
...
!     setup partner atoms (including dummies to reproduce model compound)
 ...
 &                            vs(2),at(rs)%sc(13),vs(2),1)
    ...
    else if ((resname.eq.'HIE').OR.(resname.eq.'HID').OR.(resname.eq.'HIP').OR.(resname.eq.'HEX').OR.(resname.eq.'HDX').OR.&
        &(resname.eq.'HIX')) then
    do i=2-shf,at(rs)%nsc
      sclst(i-1+shf) = at(rs)%sc(i)
    end do
...
!     setup partner atoms (including dummies to reproduce model compound)
 ...
 &                            ,vs(2),at(rs)%sc(6),vs(2),1)
    ...
    if (use_IMPSOLV.EQV..true.) then
      write(ilog,*) 'Fatal. Implicit solvent does not support re&
 &sidue ',resname,' at the moment. Check back later(a3).'
      call fexit()
    end if
...
!   the two extra 3*H
 ...
 &  (resname.eq.'HDX').OR.(resname.eq.'Y2P').OR.&
 &  (resname.eq.'S2P').OR.(resname.eq.'T2P').OR.&
 &  (resname.eq.'HEX').OR.(resname.eq.'YXP').OR.&
 &  (resname.eq.'SXP').OR.(resname.eq.'TXP').OR.&
 &  (resname.eq.'TYX')) then
    refh = at(rs)%sc(1)
    ptlst(1) = n+20-2
 ...
 &N-terminal residue ',resname,' at the moment. Check back later(a4).'
      call fexit()
    end if
  end if
!
...
! amino acid, in which case NHxCHx will be covered
 ...
 &    (resname.eq.'HIP').OR.(resname.eq.'GLX').OR.&
 &    (resname.eq.'LYX').OR.(resname.eq.'ASX').OR.&
 &    (resname.eq.'HEX').OR.(resname.eq.'Y2P').OR.&
 &    (resname.eq.'S2P').OR.(resname.eq.'T2P').OR.&
 &    (resname.eq.'HDX').OR.(resname.eq.'YXP').OR.&
 &    (resname.eq.'SXP').OR.(resname.eq.'TXP').OR.&
 &    (resname.eq.'TYX')) then
    bblst(1) = ni(rs)
    if (rs.eq.rsmol(molofrs(rs),2)) then
...
!       do nothing else
 ...
 &us (RES: ',rs,') in setup_savol().'
    ...
    if (use_IMPSOLV.EQV..true.) then
      write(ilog,*) 'Fatal. Implicit solvent does not support &
 &N-terminal residue ',resname,' at the moment. Check back later(a5).'
      call fexit()
    end if
  end if
...
!       for non-5'-residue we need the previous methyl fragment (C3* and H3*)
    ...
    if (use_IMPSOLV.EQV..true.) then
      write(ilog,*) 'Fatal. Implicit solvent does not support &
 &N-terminal residue ',resname,' at the moment. Check back later(a6).'
      call fexit()
    end if
  end if
!
...
!   do nothing
 ...
 &  (resname.eq.'LYD').OR.(resname.eq.'Y1P').OR.&
 &  (resname.eq.'S1P').OR.(resname.eq.'T1P').OR.&
 &  (resname.eq.'PRO').OR.(resname.eq.'Y2P').OR.&
 &  (resname.eq.'S2P').OR.(resname.eq.'T2P').OR.&
 &  (resname.eq.'ORN').OR.(resname.eq.'DAB').OR.&
 &  (resname.eq.'HIP').OR.(resname.eq.'GLX').OR.&
 &  (resname.eq.'LYX').OR.(resname.eq.'ASX').OR.&
 &  (resname.eq.'HDX').OR.(resname.eq.'HEX').OR.&
 &  (resname.eq.'SXP').OR.(resname.eq.'TXP').OR.&
 &  (resname.eq.'YXP').OR.(resname.eq.'TYX')) then
    refh = at(rs)%sc(1)
    ptlst(1) = n+20-2
    call genxyz(ptlst(1),cai(rs),vs(1),ni(rs),vs(2),&
 &            ci(rs),vs(2),chiral(rs))
...
!   do nothing
    ...
    if (use_IMPSOLV.EQV..true.) then
      write(ilog,*)
      write(ilog,*) 'Fatal. Implicit solvent does not support &
 &C-terminal residue ',resname,' at the moment. Check back later(a7).'
      call fexit()
    end if
  end if
!
...
!   do nothing
    ...
    if (use_IMPSOLV.EQV..true.) then
      write(ilog,*) 'Fatal. Implicit solvent does not support &
 &C-terminal residue ',resname,' at the moment. Check back later(a8).'
      call fexit()
    end if
  end if
! and now let's assemble the full partner atom list
 ...
 &    (resname.eq.'HDX').OR.(resname.eq.'HEX').OR.&
 &    (resname.eq.'TXP').OR.(resname.eq.'YXP').OR.&
 &    (resname.eq.'SXP').OR.(resname.eq.'TYX')) then
    ptlst(1+ptnr) = cai(rs)
    if (ua_model.eq.0) then
      ptlst(2+ptnr) = at(rs)%sc(1)
      ptnr = ptnr + 2
...
!   do nothing
    ...
    if (use_IMPSOLV.EQV..true.) then
      write(ilog,*) 'Fatal. Implicit solvent does not support &
 &C-terminal residue ',resname,' at the moment. Check back later(a9).'
      call fexit()
    end if
  end if
!
