...
!-----------------------------------------------------------------------
!
subroutine mcmove_pucker(rs,puckdofs,mode,ct)
!
  use iounit
  use cutoffs
  use molecule
...
!
  if ((mode.eq.0).OR.(mode.eq.1)) then
    ...
    if (ct.EQV..true.) then
      call makeref_forbb(molofrs(rs),rsmol(molofrs(rs),1))
      call sample_pucker(rs,puckdofs,cur_pucks,mode)
      call alignC_pucker(rs)
      if (use_mcgrid.EQV..true.) then
        do j=rsmol(molofrs(rs),1),rsmol(molofrs(rs),2)
          call updateresgp(j)
        end do
      end if
    else
      call makeref_forbb(molofrs(rs),rs)
      call sample_pucker(rs,puckdofs,cur_pucks,mode)
      if (use_mcgrid.EQV..true.) then
        do j=rs,rsmol(molofrs(rs),2)
          call updateresgp(j)
        end do
      end if
    end if
...
!      call internal_bound(molofrs(rs))
    ...
    if (ct.EQV..true.) then
      call getref_forbb(molofrs(rs),rsmol(molofrs(rs),1))
      if (use_mcgrid.EQV..true.) then
        do j=rsmol(molofrs(rs),1),rsmol(molofrs(rs),2)
          call updateresgp(j)
        end do
      ...
      call getref_forbb(molofrs(rs),rs)
      if (use_mcgrid.EQV..true.) then
        do j=rs,rsmol(molofrs(rs),2)
          call updateresgp(j)
        end do
    ...
    write(ilog,*) 'Fatal. Called mcmove_pucker(...) with unknown mode. &
 &Offending mode # is ',mode,'.'
    call fexit()
  end if
!
...
!----------------------------------------------------------------------
!
subroutine mcmove_pivot(rs,mode,ct)
!
  use iounit
  use cutoffs
  use molecule
...
!
  if ((mode.eq.0).OR.(mode.eq.1)) then
    ...
    if (ct.EQV..true.) then
      call makeref_forbb(molofrs(rs),rsmol(molofrs(rs),1))
      call sample_bb(rs,mode)
      call alignC_pivot(rs)
      if (use_mcgrid.EQV..true.) then
        do j=rsmol(molofrs(rs),1),rsmol(molofrs(rs),2)
          call updateresgp(j)
        end do
...
!      call internal_bound(molofrs(rs))
    ...
    write(ilog,*) 'Fatal. Called mcmove_pivot(...) with unknown mode&
 &. Offending mode # is ',mode,'.'
    call fexit()
  end if
end
...
!----------------------------------------------------------------------
!
subroutine mcmove_nuc(rs,mode,ct)
!
  use iounit
  use cutoffs
  use molecule
...
!      call internal_bound(molofrs(rs))
    ...
    write(ilog,*) 'Fatal. Called mcmove_nuc(...) with unknown mode. &
 &Offending mode # is ',mode,'.'
    call fexit()
  end if
!
...
!----------------------------------------------------------------------
!
subroutine mcmove_sugar(rs,puckdofs,mode,ct)
!
  use iounit
  use cutoffs
  use molecule
...
!
  if ((mode.eq.0).OR.(mode.eq.1)) then
    ...
    if (ct.EQV..true.) then
      call makeref_forbb(molofrs(rs),rsmol(molofrs(rs),1))
      call sample_sugar(rs,puckdofs,mode)
      call alignC_sugar(rs)
      if (use_mcgrid.EQV..true.) then
        do j=rsmol(molofrs(rs),1),rsmol(molofrs(rs),2)
          call updateresgp(j)
        end do
...
!      call internal_bound(molofrs(rs))
    ...
    write(ilog,*) 'Fatal. Called mcmove_sugar(...) with unknown mode. &
 &Offending mode # is ',mode,'.'
    call fexit()
  end if
!
...
!----------------------------------------------------------------------
!
subroutine mcmove_omega(rs,mode,ct)
!
  use iounit
  use cutoffs
  use molecule
...
!
  if ((mode.eq.0).OR.(mode.eq.1)) then
    ...
    if (ct.EQV..true.) then
      call makeref_forbb(molofrs(rs),rsmol(molofrs(rs),1))
      call sample_w(rs,mode)
      call alignC_omega(rs)
      if (use_mcgrid.EQV..true.) then
        do j=rsmol(molofrs(rs),1),rsmol(molofrs(rs),2)
          call updateresgp(j)
        end do
...
!      call internal_bound(molofrs(rs))
    ...
    write(ilog,*) 'Fatal. Called mcmove_omega(...) with unknown mode&
 &. Offending mode # is ',mode,'.'
    call fexit()
  end if
!
...
!
subroutine mcmove_other(ati,mode,ct,setalC,rsi,rsf,isnat)
!
  use iounit
  use sequen
  use atoms
...
!
  implicit none
!
  integer rs,mode,rsf,rsi,ati
  logical ct,isnat,setalC
!
...
!
  if ((mode.eq.0).OR.(mode.eq.1)) then
    ...
    call makeref_forrotlst(ati,setalC,rsi,rsf)
    call sample_other(ati,mode,ct,isnat)
    if (use_mcgrid.EQV..true.) then
      do rs=rsi,rsf
        call updateresgp(rs)
      end do
...
!      call internal_bound(molofrs(rs))
    ...
    call getref_forrotlst(ati,setalC,rsi,rsf)
    if (use_mcgrid.EQV..true.) then
      do rs=rsi,rsf
        call updateresgp(rs)
      end do
...
!----------------------------------------------------------------------
!
subroutine mcmove_fyc(rs,mode,ct)
!
  use iounit
  use cutoffs
  use molecule
...
!
  implicit none
!
  integer imol,rs,ttc,azero,nt,i,mode
  logical afalse
  RTYPE cur_other,bufy(4)
!
  afalse = .false.
  azero = 0
!
  do rs=rsmol(imol,1),rsmol(imol,2)
    if (wnr(rs).gt.0) then
      ttc = wnr(rs)
      cur_omega = omega(rs) + dc_di(imol)%incr(ttc)
      if (cur_omega.gt.180.0) cur_omega = cur_omega - 360.0
      if (cur_omega.lt.-180.0) cur_omega = cur_omega + 360.0
      call setw(rs,cur_omega,mode)
    end if
    cur_phi = phi(rs) ! may be nonsensical
    cur_psi = psi(rs) ! may be nonsensical
    if (fnr(rs).gt.0) then
      ttc = fnr(rs)
      cur_phi = phi(rs) + dc_di(imol)%incr(ttc)
      if (cur_phi.gt.180.0) cur_phi = cur_phi - 360.0
      if (cur_phi.lt.-180.0) cur_phi = cur_phi + 360.0
    end if
    if (ynr(rs).gt.0) then
      ttc = ynr(rs)
      cur_psi = psi(rs) + dc_di(imol)%incr(ttc)
      if (cur_psi.gt.180.0) cur_psi = cur_psi - 360.0
      if (cur_psi.lt.-180.0) cur_psi = cur_psi + 360.0
    end if
    if ((fnr(rs).gt.0).OR.(ynr(rs).gt.0)) then
      ...
      call setfy(rs,cur_phi,cur_psi,azero)
      if (mode.eq.1) then
    ...
    do nt=1,nnucs(rs)
      if (nucsnr(nt,rs).le.0) then
        cur_nucs(nt) = nucs(nt,rs)
        cycle
      end if
      ttc = nucsnr(nt,rs)
      cur_nucs(nt) = nucs(nt,rs) + dc_di(imol)%incr(ttc)
      if (cur_nucs(nt).gt.180.0) cur_nucs(nt) = cur_nucs(nt) - 360.0
      if (cur_nucs(nt).lt.-180.0) cur_nucs(nt) = cur_nucs(nt) + 360.0
      cur_nucflag(nt) = .true.
    end do
    ...
    do nt=1,nchi(rs)
      if (chinr(nt,rs).le.0) then
        cur_chis(nt) = chi(nt,rs)
        cycle
      end if
      ttc = chinr(nt,rs)
      cur_chis(nt) = chi(nt,rs) + dc_di(imol)%incr(ttc)
      if (cur_chis(nt).gt.180.0) cur_chis(nt) = cur_chis(nt) - 360.0
      if (cur_chis(nt).lt.-180.0) cur_chis(nt) = cur_chis(nt) + 360.0
      cur_chiflag(nt) = .true.
    end do
    if (nchi(rs).gt.0) then
  ...
  if (othidxmol(moltypid(imol)).gt.0) then
    do i=1,dc_di(imol)%maxntor
      ...
      cur_other = ztor(dc_di(imol)%recurs(i,1)) + dc_di(imol)%incr(ttc)
      if (cur_other.gt.180.0) cur_other = cur_other - 360.0
      if (cur_other.lt.-180.0) cur_other = cur_other + 360.0
      call setother(dc_di(imol)%recurs(i,1),cur_other,mode,afalse)
    end do
  end if
!
end
...
!
subroutine sample_other(ati,mode,alC,isnat)
!
  use iounit
  use zmatrix
  use movesets
...
! mode=0: sample fresh, mode=1: apply current sampling move
 ...
 &Offending mode # is ',mode,'.'
  ...
  oth = doth/RADIAN
  call quatxyz_forrotlst(ati,alC,oth)
!
end
!
!----------------------------------------------------------------------
