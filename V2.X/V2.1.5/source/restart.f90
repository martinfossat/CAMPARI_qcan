...
!
  implicit none
!
  integer i,imol,irst,freeunit,t1,t2,intdim,k,aone,azero,mt,ioms
  integer ioms2
  RTYPE eee,energy,et(MAXENERGYTERMS),force3,ettr(MAXENERGYTERMS)
  RTYPE etlr(MAXENERGYTERMS)
...
!
  ...
  if (exists.EQV..true.) then
    irst = freeunit()
    open(unit=irst,file=dumpfile(t1:t2),status='replace',iostat=ioms)
    if (ioms.ne.0) then
      ...
      open(unit=irst,file='N_'//nod(1:tl)//'_RESTART.tmp',status='replace',iostat=ioms2)
      if (ioms2.ne.0) then
    ...
    irst = freeunit()
    open(unit=irst,file=dumpfile(t1:t2),status='new',iostat=ioms)
    if (ioms.ne.0) then
  ...
  if (exists.EQV..true.) then
    ...
    if (ioms.ne.0) then
      write(ilog,*) 'An error has occurred while replacing the old restart file. NFS/Fortran incompatibility bug?'
      write(ilog,*) 'The unit number is ',irst,' (filename: ',dumpfile(t1:t2),'). Check Fortran error message.'
      open(unit=irst,file='RESTART.tmp',status='replace',iostat=ioms2)
      if (ioms2.ne.0) then
        write(ilog,*) 'Fatal. File system error. Unable to open temporary file. Check status and permissions on file system.'
        call fexit()
      else
...
!
! now internals (note we want to avoid all possible inaccuracies
! through transformation operations)
  do imol=1,nmol
...
!
  implicit none
!
  integer i,imol,irst,freeunit,t1,t2,k,intdim,ttc
  integer mt,dummy
  RTYPE eee,eee2,energy,eeek
  RTYPE force3,random
#ifdef ENABLE_MPI
  integer modstep,tl
  character(3) nod
...
! messages, and hence the calculation would be out of synch and eventually
! crash)
#ifdef ENABLE_MPI
