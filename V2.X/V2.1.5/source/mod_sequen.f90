...
! #tyler pka: an even worse violation of elegance
  logical do_pka_3

  integer nhis ! MArtin : litteraly the number of titrable histidine in the sequence
  
  integer, allocatable:: temp_which_dpg(:,:)
  integer, allocatable:: temp_nrpolnb(:,:),temp_nrpolintra(:,:),temp_nrexpolin(:,:),temp_nrexpolnb(:,:)
  integer, ALLOCATABLE :: temp_nrsin(:,:),temp_nrsnb(:,:)
  
  integer, allocatable:: his_state(:,:),his_state_save(:,:)! First dimension is the position of the histidine, second its state(0 for HE 1 for HD)

  integer, ALLOCATABLE :: temp_atin(:,:,:)
  integer, allocatable :: his_eqv_table(:)
  integer, allocatable :: spec_limit(:)
  
  integer, ALLOCATABLE:: temp_save(:),temp_bio_save(:)
  ! Martin : added for HS
  integer TOT_X,MAX_SWITCH
