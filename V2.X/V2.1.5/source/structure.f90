...
!   polars
    ...
    if (resname.EQ.'GLX') resrad(rs) = 6.2
    if (resname.EQ.'LYX') resrad(rs) = 7.6
    if (resname.EQ.'ASX') resrad(rs) = 4.8
!   chargeds
    ...
    if (resname.EQ.'S1P') resrad(rs) = 6.3
    if (resname.EQ.'T1P') resrad(rs) = 6.3
!   aromatics
    if (resname.EQ.'HIE') resrad(rs) = 6.2
    if (resname.EQ.'HID') resrad(rs) = 6.3
    if (resname.EQ.'HIP') resrad(rs) = 6.3
    if (resname.EQ.'HDX') resrad(rs) = 6.3
    if (resname.EQ.'HEX') resrad(rs) = 6.3
    if (resname.EQ.'HIX') resrad(rs) = 6.3
    if (resname.EQ.'PHE') resrad(rs) = 6.6
    if (resname.EQ.'TYR') resrad(rs) = 7.5
    if (resname.EQ.'Y1P') resrad(rs) = 10.2
    if (resname.EQ.'Y2P') resrad(rs) = 10.2
...
!   small molecules (only marginally buffered)
    ...
    if (resname.EQ.'NAX') resrad(rs) = 0.0
    if (resname.EQ.'CLX') resrad(rs) = 0.0
    if (resname.EQ.'K+ ') resrad(rs) = 0.5
    if (resname.EQ.'BR-') resrad(rs) = 0.5
    if (resname.EQ.'CS+') resrad(rs) = 0.5
    if (resname.EQ.'I- ') resrad(rs) = 0.5
...
! #tyler pka
    if (resname.EQ.'GLX') natres(rs) = 10 + 10
    if (resname.EQ.'LYX') natres(rs) = 10 + 16
    if (resname.EQ.'ASX') natres(rs) = 10 + 7
!   chargeds
    ...
    if (resname.EQ.'TYO') natres(rs) = 10 + 14 ! Martin : Deleted one atom
    if (resname.EQ.'TYR') natres(rs) = 10 + 15
    if (resname.EQ.'TYX') natres(rs) = 10 + 15
    if (resname.EQ.'CYX') natres(rs) = 10 + 5
    if (resname.EQ.'S1P') natres(rs) = 10 + 10
    if (resname.EQ.'S2P') natres(rs) = 10 + 10-1
    if (resname.EQ.'SXP') natres(rs) = 10 + 10
    if (resname.EQ.'T1P') natres(rs) = 10 + 13
    if (resname.EQ.'T2P') natres(rs) = 10 + 13-1
    if (resname.EQ.'TXP') natres(rs) = 10 + 13
    if (resname.EQ.'Y1P') natres(rs) = 10 + 20
    if (resname.EQ.'Y2P') natres(rs) = 10 + 20-1
    if (resname.EQ.'YXP') natres(rs) = 10 + 20
!   aromatics
    if (resname.EQ.'HIE') natres(rs) = 10 + 11
    if (resname.EQ.'HID') natres(rs) = 10 + 11
    if (resname.EQ.'HIP') natres(rs) = 10 + 12
    if (resname.EQ.'HEX') natres(rs) = 10 + 12
    if (resname.EQ.'HDX') natres(rs) = 10 + 12
    if (resname.EQ.'HIX') natres(rs) = 10 + 12
...
!   small molecules
    ...
    if (resname.EQ.'NAX') natres(rs) = 1
    if (resname.EQ.'CLX') natres(rs) = 1
...
!
 45   format(100000(i8,': ',a3,' |'))
 46   format(100000(g14.7,1x))
#ifdef ENABLE_MPI
  lext = 3
...
!
 58   format(g12.5,10000(1x,g14.7))
#ifdef ENABLE_MPI
  lext = 3
  if (use_REMC.EQV..true.) then
    call int2str(myrank,nod,lext)
...
!     now intermolecular terms for molecules of the same type
        ...
        do kmol=moltyp(imt,1),nmol
          ...
          if ((ens%flag.eq.5).OR.(ens%flag.eq.6)) then
            if (ismember(fluctypes,imt).EQV..true.) then
              if (ismember(ispresent,kmol).EQV..false.) then
                kk = kk + moltyp(jmt,2) - cnt2
                if ((gpc%lst(i,1)-atmol(imol,1)).ne.(gpc%lst(i,2)-atmol(jmol,1))) kk = kk + moltyp(jmt,2) - cnt2
                if (cnt.eq.moltyp(imt,2)) exit
                cycle
              end if
            end if
...
!
  if (doinst.EQV..true.) then
 666 format(i11,1x,10000000(g11.5,1x))
...
!
  ...
  iu=freeunit()
  open(unit=iu,file=fn(ii:jj),status='new')
 48   format(f11.4,1x,10000(g11.5,1x))
  do i=1,maxb
...
!
 67   format('#    Distance |',' Vol. Element |',4000('|',i3,' vs',i3,'    |'))
 66   format(f14.7,1x,4001(g14.7,1x))
#ifdef ENABLE_MPI
  lext = 3
  if (use_REMC.EQV..true.) then
    call int2str(myrank,nod,lext)
...
!
  integer i,imol
  RTYPE cto(3),ctn(3),tvec(3),qrot(4),sh(3),rmsdv
  RTYPE, ALLOCATABLE:: shcom(:,:)
!
...
!
  allocate(shcom(nmol,3))
  shcom(:,:) = 0.0
!
  if (align%mmol.eq.0) then
    do i=1,align%nr
      shcom(molofrs(atmres(align%set(i))),1) = shcom(molofrs(atmres(align%set(i))),1) + 1.0
    end do
    sh(1) = 0.0
    do i=1,nmol
      if (shcom(i,1).gt.sh(1)) then
        sh(1) = shcom(i,1)
        align%mmol = i
      end if
    end do
...
!
  do imol=1,nmol
    call shift_bound(align%mmol,imol,sh)
    shcom(imol,:) = sh(:)
  end do
!
! collect new PBC-corrected coordinates
  do i=1,align%nr
    align%curxyz(3*i-2) = x(align%set(i)) + shcom(molofrs(atmres(i)),1) - com(molofrs(atmres(i)),1)
    align%curxyz(3*i-1) = y(align%set(i)) + shcom(molofrs(atmres(i)),2) - com(molofrs(atmres(i)),2)
    align%curxyz(3*i) = z(align%set(i)) + shcom(molofrs(atmres(i)),3) - com(molofrs(atmres(i)),3)
  end do
...
!
! compute and print RMSD - note that this destroys the coordinates prior to alignment
...
!   use current (aligned) coordinates as alignment reference set
    do i=1,align%nr
      align%refxyz(3*i-2) = x(align%set(i))
      align%refxyz(3*i-1) = y(align%set(i))
      align%refxyz(3*i) = z(align%set(i))
