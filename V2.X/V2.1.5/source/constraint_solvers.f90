...
!
  ...
  use system
  use mini
  use params, ONLY: MAXBAPAR,MAXBOPAR
  use energies, ONLY: use_BOND
  use inter
!
  implicit none
!
  integer i,j,k,kk,ii,ii3,ii4,jj,mt,mt2,imol,hits(MAXVLNC*MAXVLNC),nhits,t1,t2,nidx,nangls
...
!
! disable P-SHAKE if non-SETTLE groups with massless particles exist
  do mt=1,shake_cnt
    i = nonsettlelst(mt)
...
!
  integer i,k
  RTYPE t1,t2,mvals1(2),mvals2(2),mvals4(2),mvals5(2),mvals6(2),rvals(4),rvals2(4),mvals3(2),rvals3(4)
...
!
  implicit none
! 
  integer nc,k,np,grpid,jumpidx,loi
  RTYPE maxdev,idt
  RTYPE dvec(constraints(grpid)%nr,3)
  RTYPE dvecref(constraints(grpid)%nr,3)
  ...
  RTYPE mterm1(constraints(grpid)%nr)
  RTYPE mterm2(constraints(grpid)%nr)
  RTYPE amat(constraints(grpid)%maxcpl,constraints(grpid)%nr)
  RTYPE rvec(constraints(grpid)%nr,2)
...
!
  constraints(grpid)%iters = constraints(grpid)%iters + lincs_order
  constraints(grpid)%itercnt = constraints(grpid)%itercnt + 1
!
end
!
