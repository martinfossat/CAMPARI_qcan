...
!
  RTYPE, ALLOCATABLE:: atr(:),atvol(:),atsav(:),atstv(:),mass(:)
  RTYPE, ALLOCATABLE:: svte(:),svbu(:),atbvol(:),atsavmaxfr(:)
  RTYPE, ALLOCATABLE:: atsavred(:),atsavprm(:,:),atq(:),scrq(:)
  
! Martin 
  RTYPE, ALLOCATABLE:: atsavred_limits(:,:), atq_limits(:,:)
  RTYPE, ALLOCATABLE:: atsavprm_limits(:,:,:),atstv_limits(:,:)
  
  integer, allocatable:: temp_nrsdi(:,:),temp_nrsba(:,:),temp_nrsbl(:,:),temp_nrsimpt(:,:)
  
  integer, ALLOCATABLE:: seq_q_state(:),seq_q_state_save(:)! Martin : those two are only here for the purpose of HS titration
  integer, ALLOCATABLE:: seq_q_sign(:),seq_q_H_dir(:) ! Martin : those two are only here for the purpose of HS titration
  integer, ALLOCATABLE:: tautomer(:) !This array is supposed to be zero for everything but uncharged histidine where it 
  !can be 1 (HID)or 2(HIE)
  
  integer, ALLOCATABLE:: q_switch(:)
  
! Martin added :
  RTYPE, ALLOCATABLE:: atr_limits(:,:),atvol_limits(:,:)
  RTYPE, ALLOCATABLE:: x_limits(:,:),y_limits(:,:),z_limits(:,:)
