...
! &           par_IMPSOLV(11)*sexv ! Commented out again, the sexv is never initialized, are relies on the
!         fact that par_IMPSOLV is 0 to be canceled out, which is very bad practice.! However, in V2 this is not used,
! so potentially not really a problem
...
!    at(rs)%fosgrp(at(rs)%nfosgrps)%ats(2) = at(rs)%sc(at(rs)%nsc)
!    at(rs)%fosgrp(at(rs)%nfosgrps)%wts(2) = 1.0/(1.0*hh)
!    which_fosg(at(rs)%sc(at(rs)%nsc)) = at(rs)%nfosgr
! phosphoserine (phosphate dominates, strategy similar to 5' phosphate in nucleotides)
! Martin : have not deleted the extra atom yet
  else if (resname.eq.'S2P') then
    at(rs)%nfosgrps = at(rs)%nfosgrps + 1
