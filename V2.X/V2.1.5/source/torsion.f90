...
!  write(idihed,*) '# Step Number      ',stringy(1:ttc*8) 
 34 format('#_Step_No.__|',20000(a10))
 35 format('#_Step_No.__|',20000(i9,'|'))
#ifdef ENABLE_MPI
  if (use_REMC.EQV..true.) then
...
subroutine z_secondary(imol,zav,zbv,zav_vector,zbv_vector)
!
  use iounit
  use sequen
  use energies
...
!
  implicit none
!
  integer i,j,imol,nress
  RTYPE dis,vv(2),ww(2),uu(2),zav,zbv
  RTYPE zav_vector(maxval(rsmol(:,2)-rsmol(:,1))+1)
  RTYPE zbv_vector(maxval(rsmol(:,2)-rsmol(:,1))+1)
...
!     this residue is fully alpha
      zav = zav + 1.0
! Add the helicity to the z_alpha_vector #tyler
      zav_vector(i-rsmol(imol,1)+1) = 1.0
    else
...
! Add the helicity to the z_alpha_vector #tyler
      zav_vector(i-rsmol(imol,1)+1) = exp(-par_ZSEC2(4)*(uu(1)**2+uu(2)**2))
    end if
!   now beta
    ...
    if (dis.le.par_ZSEC2(7)) then
      zbv = zbv + 1.0
! Add the helicity to the z_beta_vector #tyler
      zbv_vector(i-rsmol(imol,1)+1) = 1.0
    else
...
! Add the helicity to the z_beta_vector #tyler
      zbv_vector(i-rsmol(imol,1)+1) = exp(-par_ZSEC2(8)*(uu(1)**2+uu(2)**2))
    end if
  end do
!
  zav = zav/(1.0*nress)
...
! normalize
 ...
 44   format(g12.4,1x,20000(g14.6))
#ifdef ENABLE_MPI
  lext = 3
  if (use_REMC.EQV..true.) then
